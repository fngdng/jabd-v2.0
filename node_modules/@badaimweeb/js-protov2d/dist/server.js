...
export class ProtoV2dServer extends EventEmitter {
    ...
    handleWSConnection(client, header) {
    ...
    handleWrappedConnection(wc) {
        ...
        // Handle connection
        let handleIncomingPacket = async (rawPacket) => {
            try {
                if (state.currentPacket === 0) {
                    ...
                    let handshakePacket = decode(rawPacket.slice(1));
                    ...
                    if (handshakePacket[1] === 1) {
                        ...
                        if (handshakePacket[3] === 2) {
                            ...
                            this.debug(`sending handshake v2 packet ${state.currentPacket}`);
                            wc.send(joinUint8Array([0x02, 0x02, 0x01], fullPublicKey, signatureClassic, signaturePQ, state.challenge2, ...(handshakePacket[3] === 1 ? [this.classicKeyPair.publicKey, this.pqKeyPair.publicKey] : [await this.publicKeyHash])));
                        }
                        else {
                            // Invalid state
                            closeConnection();
                ...
                switch (state.version) {
                    case 1: {
                        ...
                        switch (state.currentPacket) {
                            case 1: {
                                let enc = decode(rawPacket.slice(1));
                                if (enc[0] !== 3) {
                                    // Invalid state
                                    closeConnection();
                                    return;
                                ...
                                encryptionKeyPQ = await crypto.subtle.importKey("raw", sharedSecret.sharedSecret, "AES-GCM", true, ["encrypt", "decrypt"]);
                            ...
                            case 2: {
                                let enc = decode(await aesDecrypt(rawPacket.slice(1), encryptionKeyPQ, false));
                                if (enc[0] !== 5) {
                                    // Invalid state
                                    closeConnection();
                                    return;
                                ...
                                if (this.sessions.has(sessionID) && !this.sessions.get(sessionID).closed) {
                                    ...
                                    wc.send(joinUint8Array([0x02], encryptedPacket));
                                    session = new ProtoV2dSession(sessionID, 1, false, wc, [encryptionKeyPQ], this.config.pingTimeout || 10000, this.config.pingInterval || 15000, this.config.pingAvgCount || 10);
                        ...
                        switch (state.currentPacket) {
                            case 1: {
                                ...
                                if (state.encryption) {
                                    ...
                                    encryptionKeyClassic = await crypto.subtle.importKey("raw", sharedSecretClassic, "AES-GCM", true, ["encrypt", "decrypt"]);
                                    encryptionKeyPQ = await crypto.subtle.importKey("raw", sharedSecretPQ, "AES-GCM", true, ["encrypt", "decrypt"]);
                                ...
                                if (this.sessions.has(sessionID) && !this.sessions.get(sessionID).closed) {
                                    ...
                                    session = new ProtoV2dSession(sessionID, 2, false, wc, [encryptionKeyPQ, encryptionKeyClassic].filter(filterNull), this.config.pingTimeout || 10000, this.config.pingInterval || 15000, this.config.pingAvgCount || 10);
                                    this.sessions.set(sessionID, session);
                                    // session lifecycle management
                                    this._handleSessionLifecycle(session);
                                    this.emit("connection", session);
    ...
    _handleSessionLifecycle(session) {
