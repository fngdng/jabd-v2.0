 ...
 * By default, `reconnectionTime` is 5s, this means that it will retry connection every 5s if the connection is closed; `timeout` is 10s.
 */
export async function connectWithCustomConnect(config, connectFunc) {
    let sessionKey = void 0, sessionID = void 0;
    let sessionObject = void 0;
    ...
    for (let r = 0; r < (config.maxInitialRetries ?? Infinity); r++) {
        try {
            ...
            async function onWCDisconnect(explict, reason) {
                ...
                for (;;)
                    try {
                        ...
                        await new Promise(r => setTimeout(r, config.reconnectionTime || 5000));
                    }
            }
            sessionObject.wc.once("close", onWCDisconnect);
            return baseSession;
            ...
            if (e instanceof NRError)
                throw e;
            err = e;
            if (r === (config.maxInitialRetries ?? Infinity) - 1)
                break;
            await new Promise(r => setTimeout(r, config.reconnectionTime || 5000));
        }
    }
    ...
    return new Promise(async (resolve, reject) => {
        ...
        async function handleData(e) {
            ...
            if (eData instanceof ArrayBuffer)
                d = new Uint8Array(eData);
            else if (typeof eData === "string") {
                let enc = new TextEncoder();
                d = enc.encode(eData);
            }
            else if (eData instanceof Blob)
                d = new Uint8Array(await eData.arrayBuffer()); // redundant
            else if (eData instanceof Buffer)
                d = new Uint8Array(eData); // redundant
 ...
 */
export function connectWrapped(config) {
    return new Promise(async (resolve, reject) => {
        ...
        if (!config.existingData || !config.existingData.sessionKey || !config.existingData.sessionID) {
            reject(new NRError("Missing session key/ID"));
            return;
        }
        if (!["auto", "disabled", "forced"].includes(config.handshakeV1 || "auto")) {
            reject(new NRError("v1 handshake option is invalid"));
            return;
        }
        ...
        function onCloseWC(_explict, reason) {
            reject(new Error("Socket closed before handshake succeeded" + (reason ? ": " + reason : "")));
            wc.removeListener("rx", onIncomingData);
            wc.removeListener("close", onCloseWC);
        }
        async function onIncomingData(data) {
            if (!state.handshaked && data[0] !== 0x02) {
                rejectHandshake("Invalid data during handshake");
                return;
            }
            if (state.version === 0) {
                ...
                if (data[1] === 0x94)
                    ...
                    rejectHandshake("Unknown version");
                    return;
                }
                ;
            ...
            if (state.version === 1 && config.handshakeV1 === "disabled") {
                rejectHandshake("v1 handshake is disabled");
                return;
            }
            switch (state.version) {
                case 1: {
                    switch (state.currentPacket) {
                        case 1: {
                            let packet = decode(data.slice(1));
                            if (packet[0] !== 2) {
                                rejectHandshake("Invalid packet");
                                return;
                            }
                            ...
                            if (!noVerify) {
                                ...
                                if (!pkValues.some(v => {
                                    if (v[0])
                                        return exactArray(v[1], signaturePKH);
                                    else
                                        return exactArray(v[1], signaturePK);
                                })) {
                                    rejectHandshake("No public key matches");
                                    return;
                                }
                                ...
                                if (signature[64] !== 0xf3 || signature[65] !== 0x11) {
                                    rejectHandshake("Invalid signature");
                                    return;
                                }
                                let verified1 = ed25519.verify(signatureClassic, pqPK256256, signaturePKClassic);
                                if (!verified1) {
                                    rejectHandshake("New public key classic signature verification failed");
                                    return;
                                }
                                let verified2 = await dilithium5.verify(signaturePQ, pqPK256256, signaturePKPQ);
                                if (!verified2) {
                                    rejectHandshake("New public key PQ signature verification failed");
                                    return;
                                }
                            }
                        ...
                        case 2: {
                            let packet = decode(await aesDecrypt(data.slice(1), encryptionKeyPQ, false));
                            if (packet[0] !== 4) {
                                rejectHandshake("Invalid packet");
                                return;
                            }
                            ...
                            switch (data[2]) {
                                case 0x01: {
                                    ...
                                    if (!noVerify) {
                                        ...
                                        if (!verified1) {
                                            rejectHandshake("New public key classic signature verification failed", true);
                                            return;
                                        }
                                        let verified2 = await dilithium5.verify(signaturePQ, exchangeFull, pqPart);
                                        if (!verified2) {
                                            rejectHandshake("New public key PQ signature verification failed", true);
                                            return;
                                        }
                                    }
                                    ...
                                    state.currentPacket = 2;
                                    wc.send(joinUint8Array([0x02, 0x03], classicRandomPublic, pqData.ciphertext, await aesEncrypt(await aesEncrypt(joinUint8Array(sessionID, sessionSignatureClassic, sessionSignaturePQ), encryptionKeyPQ, true), encryptionKeyClassic, true)));
                                ...
                                case 0x02: {
                                    ...
                                    if (!config.disableEncryption) {
                                        rejectHandshake("Server send non-encryption, but client does not disable encryption.");
                                        return;
                                    }
                                ...
                                case 0x03: {
                                    ...
                                    rejectHandshake("Server does not allow disabling encryption");
                                    return;
                                }
                                case 0x04: {
                                    // Version mismatch
                                    rejectHandshake(`Server does not support client version (advertised: ${decode(data.slice(3)).join(", ")}; client supported: 2)`);
                                    return;
                                }
                                default: {
                                    rejectHandshake("Invalid packet");
